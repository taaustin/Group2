...
class MD_DGM_APP:
    ...
    def add_link(self, fileName):
        ...
        link.connect("clicked", self.show_img, fileName)
        link.show()
        files.add(link)
    ...
    def shapefile_thread(self, *args):
        if(args[0] == "zcta_map.gif"):
            GLib.idle_add(self.update_log, "Rendering ZCTAs...")
            img = zipdistrict.cluster("zctas")
        if(args[0] == "districts_map.gif"):
            GLib.idle_add(self.update_log, "Rendering districts...")
            img = zipdistrict.cluster("districts")
        img.save(args[0])
